...
- one frozen arm + case ID
- deterministic ascending PDDR context order
- the complete Markdown text of each selected record
- exact system/user messages
- context and full-prompt SHA-256 hashes

Gold action, gold source record, rationale, and `source_case_id` are not
included in the request pack.
...
selection remains a separate pre-run provenance decision; no model output is
